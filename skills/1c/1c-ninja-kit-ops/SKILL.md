...
- Контуры в ките: статический **mcp-1c-autumn** (порт lekot/mcp-1c на autumn-mcp), живой **1c-mcp-toolkit :6003**, LLM-ядро **1c-buddy :6002**. **bslc — изъят** («БЕЗ НАДОБНОСТИ», архив `C:\hermes\bsl-server-fail`). **prime-agent (pi-bridge) — отложен** (сырой/дублирующий, архив `C:\hermes\prime-fail`).
