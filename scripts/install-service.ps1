...
$log = "C:\hemes\tools\run\svc-install.log"
...
$script  = "C:\hemes\tools\scripts\pi-bridge.py"

sc.exe delete pi-bridge 2>$null | Out-Null
& $nssm install pi-bridge $pythonw $script
"nssm install exit=$LASTEXITCODE"
& $nssm set pi-bridge AppDirectory "C:\hemes"
& $nssm set pi-bridge AppExit Default Restart
& $nssm set pi-bridge AppStdout "C:\hemes\tools\run\pi-svc.out.log"
& $nssm set pi-bridge AppStderr "C:\hemes\tools\run\pi-svc.err.log"
